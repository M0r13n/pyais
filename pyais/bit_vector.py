...
Uses pre-computed lookup tables and stores the entire payload as a single
arbitrary-precision int so that every field extraction is a constant-
time shift-and-mask operation.
"""
import typing as t
...
_PAYLOAD_ARMOR: t.Final[tuple[int, ...]] = tuple(
    (c - 48 - 8 if c - 48 > 40 else c - 48) if 48 <= c <= 119 else 0
    for c in range(256)
)
...
def _mask(n: int) -> int:
    """Return a bitmask of *n* set bits.  Uses the lookup table when
    possible, falls back to computation for very wide fields."""
    if n < 257:
        return _MASK[n]
    return (1 << n) - 1
...
class BitVector:
    ...
    def __init__(self, data: bytes, pad: int = 0) -> None:
        armor = _PAYLOAD_ARMOR
        value = 0
        for byte in data:
            value = (value << 6) | armor[byte]
    ...
    def get_num(self, start: int, width: int, signed):
        if signed:
    ...
    def get(self, start: int, width: int) -> int:
        ...
        only the available bits are returned (matching the old bytearray
        slicing behaviour).
        """
    ...
    def get_bytes(self, start, width):
        if width <= 0 or start >= self._length:
            return b""
