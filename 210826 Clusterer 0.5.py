...
while n_viagens_sem_cluster != 0:
    ...
    while True and nn == '1':
        ...
        #               "origin_lon",  "destiny_lon",
        #               "origin_lat",  "destiny_lat"])
        ...
        while True and n_ligacoes < N_min:
            ...
            if viagem != A2:
                ...
                distance = (float(viagem[5]) - float(A2[5])) ** 2 + (np.log(np.tan(math.radians((math.pi / 4) + (float(viagem[7]) / 2)))) - np.log(np.tan(math.radians((math.pi / 4) + (float(A2[7]) / 2))))) ** 2
                    ...
                    if n_linha > n_viagens_cluster_analizadas:
                        ...
                        while True:
                            ...
                            distance = (float(A_cluster[5]) - float(A2[5])) ** 2 \
                                       + (np.log(np.tan(math.radians((math.pi / 4) + (float(A_cluster[7]) / 2)))) - np.log(np.tan(math.radians((math.pi / 4) + (float(A2[7]) / 2))))) ** 2
                            if distance <= Epsilon ** 2 and distance != float(0):
...
while n_viagens_sem_cluster != 0:
    ...
    while True and nn == '1':
        ...
        while True and n_ligacoes < N_min:
            ...
            if viagem != A2:
                # ver se a viagem se liga a esta na
                distance = (float(viagem[6]) - float(A2[6])) ** 2 + (np.log(np.tan(math.radians((math.pi / 4) + (float(viagem[8]) / 2)))) - np.log(np.tan(math.radians((math.pi / 4) + (float(A2[8]) / 2))))) ** 2
                if distance <= Epsilon ** 2:
                    n_ligacoes = n_ligacoes + 1
        fid2.close()
                ...
                while True and mmm == 0:
                    ...
                    if n_linha > n_viagens_cluster_analizadas:
                        ...
                        while True:
                            ...
                            # you can access the line - in var A2
                            # ver se a viagem se liga a esta ligada à viagem
                            distance = (float(A_cluster[6]) - float(A2[6])) ** 2 \
                                       + (np.log(np.tan(math.radians((math.pi / 4) + (float(A_cluster[8]) / 2)))) - np.log(np.tan(math.radians((math.pi / 4) + (float(A2[8]) / 2))))) ** 2
                            if distance <= Epsilon ** 2 and distance != float(0):
                                # estão esta viagem faz parte do cluster
                                n_viagens_em_cluster = n_viagens_em_cluster + 1
                                # vamos escrever no ficheiro
